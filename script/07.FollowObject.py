...
class FrankTask(OmniBase):
    ...
    def set_up_scene(self):
        ...
        add_reference_to_stage(usd_path=asset_path, prim_path="/{}".format("Franka"))
        ee_frame = "panda_hand"; 
        ...
        self._robot = self.env.scene.add(
            SingleManipulator(prim_path="/{}".format("Franka"), name="{}".format("Franka"), end_effector_prim_name=ee_frame)
        )
    ...
    async def _on_follow_target_event_async(self, val):
        if val:
            await self.env.play_async()
            self.env.add_physics_callback("sim_step", self._on_follow_target_simulation_step)
        else:
            self.env.remove_physics_callback("sim_step")
        return

    def _on_follow_target_simulation_step(self):
        ...
        actions = self._controller.forward(
            target_end_effector_position=observations["object"]["position"],
            target_end_effector_orientation=observations["object"]["orientation"],
        )
    ...
    def main(self):
        ...
        while simulation_app.is_running():
            ...
            self._on_follow_target_simulation_step()
